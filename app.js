...
/* ================================================================
   MODERN LINK & ICON BUTTON SYSTEM
   ================================================================ */
const PLATFORM_META = {
  ...
  messenger: { name: "Messenger",   color: "#A855F7", bg: "rgba(168, 85, 247, 0.12)",  border: "rgba(168, 85, 247, 0.28)" },
  github:    { name: "GitHub",      color: "#E6EDF3", bg: "rgba(230, 237, 243, 0.10)", border: "rgba(230, 237, 243, 0.22)" },
  email:     { name: "Email",       color: "#FBBF24", bg: "rgba(251, 191, 36, 0.12)",  border: "rgba(251, 191, 36, 0.28)" },
...
// Only allow safe schemes to ever reach an href — blocks javascript:/data: etc.
...
function modernIconBadgeHtml(platformKey, size = 42) {
  ...
  return `<div class="relative flex items-center justify-center flex-shrink-0 rounded-[13px] transition-all duration-200"
...
function bannerSkeletonHtml() {
  return `<div class="mb-4 animate-pulse">
    <div class="w-full rounded-2xl" style="height:150px;background:#1C2540;"></div>
...
function postViewScreenHtml(post) {
  ...
  links.forEach((l) => {
    html += modernLinkCardHtml({ url: l.url, title: l.title || "Watch tutorial / preview video", platformKey: l.icon || detectPlatformKey(l.url), subtitle: "Tap to watch video" });
  });
  html += `</div></div>`;
  return html;
}
...
function linkRowHtmlForPost(entry, idx) {
  return `<div class="bg-bgdeep border border-bd rounded-lg p-3 mb-2.5" data-link-idx="${idx}">
    <div class="flex gap-2 mb-2">
      ${modernIconBadgeHtml(entry.icon || detectPlatformKey(entry.url), 40)}
      <select class="${inputCls} pe-link-platform" data-idx="${idx}">${LINK_PLATFORMS.map(([k, label]) => `<option value="${k}" ${entry.icon === k ? "selected" : ""}>${label}</option>`).join("")}</select>
      ${dangerIconBtn({ action: "pe-remove-link", id: String(idx), size: 40 })}
    </div>
    <input class="${inputCls} flex-1 pe-link-title mb-2" data-idx="${idx}" value="${escAttr(entry.title)}" placeholder="Name e.g. Watch on YouTube" />
...
function bindPostEditorInputs() {
  ...
  document.querySelectorAll(".pe-link-platform").forEach((el) => el.addEventListener("change", (e) => { postEditorDraft.links[+e.target.dataset.idx].icon = e.target.value; render(); }));
...
function platformSheetHtml() {
  ...
  return `<div data-action="close-platform-sheet" class="fixed inset-0 z-[220] flex items-end justify-center" style="background:rgba(0,0,0,0.7);">
    <div data-action="noop" class="w-full max-w-[480px] bg-panel rounded-t-[20px] pt-[18px] pb-7 fade-in" style="max-height:75vh;overflow-y:auto;">
      ...
      ${LINK_PLATFORMS.map(([k, label]) => `<button data-action="pick-platform" data-id="${k}" class="w-full flex items-center gap-3 px-[18px] py-3 border-t border-bd bg-transparent border-l-0 border-r-0 border-b-0 text-left">
        ${modernIconBadgeHtml(k, 34)}
        <span class="flex-1 font-inter text-[15px]">${label}</span>
        <span class="rounded-full flex-shrink-0" style="width:20px;height:20px;border:2px solid ${linkDraft.platform === k ? PLATFORM_META[k].color : "#3A445E"};display:flex;align-items:center;justify-content:center;">${linkDraft.platform === k ? `<span class="rounded-full" style="width:10px;height:10px;background:${PLATFORM_META[k].color};"></span>` : ""}</span>
      </button>`).join("")}
    </div>
  </div>`;
}
...
document.addEventListener("click", async (e) => {
  ...
  switch (action) {
    ...
    case "pe-add-link": if (postEditorDraft) { postEditorDraft.links.push({ id: "l" + Date.now(), title: "", url: "", icon: "other" }); render(); } break;
