...
function homeHeaderHtml() {
  ...
  return `<div id="site-header" class="sticky top-0 z-20" style="background:#0A0E17;transition:background-color .2s ease, backdrop-filter .2s ease;">
    <div class="flex items-center gap-2 px-4 pt-2.5 pb-2">
      ${headerLogoHtml(32)}
...
function backHeaderHtml(title, backAction = "nav", backId = "home", rightHtml = "") {
  return `<div class="flex items-center gap-2 px-4 pt-2.5 pb-2">
    ${iconBtn({ action: backAction, id: backId, icon: ICONS.arrowLeft(), size: 32 })}
...
function bottomRailHtml(items) {
  return `<div class="fixed left-1/2 bottom-0 z-40 flex" style="transform:translateX(-50%);width:100%;max-width:480px;background:rgba(10,14,23,.96);border-top:1px solid #232D48;padding:7px 4px calc(7px + env(safe-area-inset-bottom, 0px));backdrop-filter:blur(10px);">
    ${items.map((item) => item.label
      ...
      : `<button data-action="${item.action}" data-id="${item.id}" class="flex-1 flex items-center justify-center relative" style="padding:8px 0;">
        ${item.badge > 0 ? `<span class="absolute rounded-full bg-coral text-white flex items-center justify-center font-inter" style="top:2px;right:26%;min-width:15px;height:15px;font-size:9px;padding:0 3px;">${item.badge}</span>` : ""}
        <span class="flex items-center justify-center rounded-xl" style="width:40px;height:34px;color:${item.active ? "#3E8EFF" : "#8A93AC"};background:${item.active ? "rgba(62,142,255,.14)" : "transparent"};">${item.icon}</span>
      </button>`
    ).join("")}
  </div>`;
}
...
function footerHtml() {
  ...
  return `<div class="mt-2 bg-panel border-t border-bd rounded-t-[20px] px-5 pt-6 flex flex-col items-center gap-3.5" style="padding-bottom:calc(28px + 72px + env(safe-area-inset-bottom, 0px));">
