...
function accountScreenHtml() {
  const account = state.session.account;
  return `${backHeaderHtml("My profile", "nav", "home")}
  <div class="px-4 pt-2 pb-24">
    ${profileEditorHtml(profileEditorDraft || account, true)}
    <button data-action="confirm-logout" class="w-full text-center mt-5 font-inter text-xs text-tmuted border border-bd rounded-full py-2.5">Log out</button>
  </div>`;
}
...
function renderInner() {
  ...
  switch (screen) {
    ...
    case "account":
      if (!isLoggedInCreator) { html = creatorAuthScreenHtml(); break; }
      html = accountScreenHtml(); showBottomNav = true; break;
