...
function profileScreenHtml(account) {
  ...
  let html = "";
  if (isOwn) {
    html += `<div class="flex items-center justify-end gap-2 px-4 pt-4 pb-2.5">
      <button data-action="share-profile" data-id="${account.id}" class="w-9 h-9 rounded-xl border border-bd flex items-center justify-center">${ICONS.share()}</button>
      <button data-action="confirm-logout" class="w-9 h-9 rounded-xl border border-bd flex items-center justify-center">${ICONS.logOut()}</button>
    </div>`;
  } else {
    html += backHeaderHtml("", "nav", "home", `<button data-action="share-profile" data-id="${account.id}" class="w-9 h-9 rounded-xl border border-bd flex items-center justify-center">${ICONS.share()}</button>`);
  }

  html += `<div class="px-5 pb-5" style="margin-top:-6px;">
    <div class="flex items-center gap-4">
      <button data-action="${isOwn ? "open-photo-sheet" : "noop"}" class="relative rounded-full p-0 border-none bg-transparent flex-shrink-0">
        ${avatarHtml(account.name, account.avatar, 84)}
        ${isOwn ? `<span class="absolute flex items-center justify-center rounded-full text-white" style="right:-2px;bottom:-2px;width:26px;height:26px;background:#3E8EFF;border:2.5px solid #0A0E17;">${ICONS.plus()}</span>` : ""}
      </button>
      <div class="flex-1 min-w-0">
        <button data-action="${isOwn ? "open-switch-account" : "noop"}" class="flex items-center gap-1.5 max-w-full">
        ...
        ${isOwn ? `<button data-action="nav" data-id="editAccount" class="mt-2.5 rounded-full font-sora font-semibold text-[13px] px-4 py-1.5" style="background:#232D48;color:#F3F5F9;">Edit profile</button>` : ""}
      </div>
    </div>
    ${bioLines ? `<div class="mt-3.5 text-tmuted font-inter text-sm leading-relaxed whitespace-pre-wrap">${esc(bioLines)}</div>` : ""}
...
function editBioScreenHtml() {
  ...
  <div class="px-5 pt-3">
    ...
    <div class="font-inter text-[12.5px] text-tfaint mb-4">Up to 5 lines. Extra lines will be trimmed automatically.</div>
    <textarea id="eb-bio" maxlength="300" class="w-full bg-panelalt border-none rounded-xl px-4 py-3.5 font-inter text-[15px]" style="min-height:130px;">${esc(a.bio || "")}</textarea>
    <div class="text-right font-inter text-xs text-tfaint mt-1.5" id="eb-count">${lineCount}/5 lines</div>
  </div>`;
}
...
function bindCharCounter(inputId, counterId, max) {
  ...
  if (el && counter && inputId === "eb-bio") {
    el.addEventListener("input", () => {
      ...
      counter.textContent = `${lines.length}/5 lines`;
      counter.style.color = lines.length > 5 ? "#FF5D6C" : "";
    });
  } else if (el && counter) {
...
function renderInner() {
  ...
  if (document.getElementById("eb-bio")) bindCharCounter("eb-bio", "eb-count", 5);
...
async function saveBio() {
  const a = state.session.account;
  const val = document.getElementById("eb-bio").value.split("\n").slice(0, 5).join("\n");
