  ...
  return `<div id="site-header" class="sticky top-0 z-20 pb-1" style="background:#0A0E17;transition:background-color .2s ease, backdrop-filter .2s ease;">
    <div class="flex items-center gap-2.5 px-4 pt-4 pb-2.5">
      ...
      <div class="flex flex-col justify-center leading-tight min-w-0">
        <div class="font-sora font-extrabold text-[18px] tracking-tight truncate">${esc(b.siteName)}</div>
        <div class="font-inter text-[11px] text-tmuted truncate">${esc(HEADER_TAGLINE)}</div>
      </div>
...
function bottomRailHtml(items) {
  return `<div class="fixed left-1/2 bottom-0 z-40 flex" style="transform:translateX(-50%);width:100%;max-width:480px;background:rgba(10,14,23,.96);border-top:1px solid #232D48;padding:9px 2px 18px;backdrop-filter:blur(10px);">
    ${items.map((item) => `<button data-action="${item.action}" data-id="${item.id}" class="flex-1 flex flex-col items-center gap-1 font-inter relative" style="font-size:10px;color:${item.active ? "#3E8EFF" : "#8A93AC"};">
...
function publicBottomNavHtml(activeScreen) {
  ...
  return bottomRailHtml([
    { action: "nav", id: "home", label: "Home", icon: ICONS.home(), active: activeScreen === "home" },
    { action: "open-explore", id: "", label: "Explore", icon: ICONS.search(), active: state.ui.exploreOpen },
    { action: "nav", id: gate("submit"), label: "Submit", icon: ICONS.plus(), active: activeScreen === "submit" },
    { action: "nav", id: gate("favorites"), label: "Favorites", icon: ICONS.heart(activeScreen === "favorites"), active: activeScreen === "favorites" },
    { action: "nav", id: gate("account"), label: "Profile", icon: ICONS.userCircle(), active: activeScreen === "account" },
  ]);
}
function footerHtml() {
  const b = state.branding;
...
function profileScreenHtml(account) {
  ...
  const editBtn = isOwn ? iconBtn({ action: "nav", id: "editAccount", icon: ICONS.pencil() }) : "";
  if (!account.profilePublic) {
    return `${backHeaderHtml("Profile", "nav", "home", editBtn)}<div class="flex flex-col items-center px-5 py-10">
  ...
  let html = backHeaderHtml("", "nav", "home", editBtn);
  ...
  <div class="px-5 pt-1 pb-10">
    <div class="rounded-[26px] overflow-hidden shadow-2xl">
      ...
      <div class="px-6 pt-7 pb-8 bg-panel" style="margin-top:-28px;border-radius:26px 26px 0 0;">
        ...
        ${fieldWrap("Email or username", `<input id="ca-identifier" class="${inputCls}" placeholder="you@example.com or username" />`)}
...
function photoLightboxHtml() {
  if (!state.ui.photoViewOpen) return "";
  const a = state.session.account;
  return `<div data-action="close-photo-view" class="fixed inset-0 z-[210] flex items-center justify-center p-6" style="background:rgba(0,0,0,0.9);">
    <img src="${escAttr(a.avatar)}" class="max-w-full max-h-full rounded-2xl object-contain" />
  </div>`;
}
function editAccountScreenHtml() {
  ...
  <div class="px-5 pt-2 pb-10">
    ...
    <div class="bg-panel border border-bd rounded-2xl mb-4 overflow-hidden">
      <button data-action="nav" data-id="editLinks" class="w-full flex items-center justify-between px-4 py-3.5 border-b border-bd">
      ...
      <button data-action="toggle-account-visibility" class="w-full flex items-center justify-between px-4 py-3.5">
        <span class="font-inter text-sm text-tmuted">Profile visibility</span>
        <span class="font-sora font-semibold text-sm">${a.profilePublic ? "Public" : "Private"}</span>
      </button>
    </div>
...
function editBioScreenHtml() {
  ...
  <div class="px-5 pt-3">
    <div class="font-sora font-extrabold text-2xl mb-4">Bio</div>
    <textarea id="eb-bio" maxlength="150" class="w-full bg-panelalt border-none rounded-xl px-4 py-3.5 font-inter text-[15px]" style="min-height:110px;">${esc(a.bio || "")}</textarea>
    <div class="text-right font-inter text-xs text-tfaint mt-1.5" id="eb-count">${(a.bio || "").length}/150</div>
  </div>`;
}
function editLinksScreenHtml() {
  ...
  const links = a.links || [];
  return `<div class="flex items-center gap-2.5 px-4 pt-4 pb-2.5">
    ${iconBtn({ action: "nav", id: "editAccount", icon: ICONS.arrowLeft() })}
    <div class="flex-1 font-sora font-bold text-base">Links</div>
  </div>
  <div class="px-4 pt-1 pb-10">
    <button data-action="nav" data-id="editLink/new" class="w-full flex items-center gap-3.5 py-3">
    ...
    ${links.map((l, idx) => `<button data-action="nav" data-id="editLink/${idx}" class="w-full flex items-center gap-3.5 py-3 border-t border-bd text-left">
    ...
    <div class="font-inter text-[12.5px] text-tfaint mt-4 leading-relaxed">Your links are visible to everyone who visits your profile.</div>
  </div>`;
}
function editLinkFormScreenHtml(param) {
  const isNew = !param || param === "new";
  const idx = isNew ? -1 : parseInt(param, 10);
  const links = state.session.account.links || [];
  const link = isNew ? { url: "", label: "" } : (links[idx] || { url: "", label: "" });
  return `<div class="flex items-center gap-2.5 px-4 pt-4 pb-2.5">
    ${iconBtn({ action: "nav", id: "editLinks", icon: ICONS.arrowLeft() })}
    <div class="flex-1 font-sora font-bold text-base">${isNew ? "Add link" : "Edit link"}</div>
    <button data-action="save-link" data-id="${idx}" class="text-tprimary bg-transparent border-none">${ICONS.check()}</button>
  </div>
  <div class="px-5 pt-2">
    <input id="el-url" class="w-full bg-transparent border border-bd rounded-xl px-4 py-3.5 font-inter text-[15px] mb-3" placeholder="URL (https://...)" value="${escAttr(link.url)}" />
    <input id="el-title" class="w-full bg-transparent border border-bd rounded-xl px-4 py-3.5 font-inter text-[15px]" placeholder="Title" value="${escAttr(link.label)}" />
    <div id="el-error" class="text-coral text-xs mt-3 font-inter"></div>
    ${!isNew ? `<button data-action="delete-link" data-id="${idx}" class="w-full text-center mt-6 font-inter text-sm bg-transparent border-none" style="color:#FF5D6C;">Remove link</button>` : ""}
  </div>`;
}
function bindCharCounter(inputId, counterId, max) {
  const el = document.getElementById(inputId);
  const counter = document.getElementById(counterId);
  if (el && counter) el.addEventListener("input", () => { counter.textContent = `${el.value.length}/${max}`; });
...
function renderInner() {
  ...
  switch (screen) {
    ...
    case "editLink":
      html = isLoggedInCreator ? editLinkFormScreenHtml(param) : creatorAuthScreenHtml(); break;
  ...
  const page = `<div class="flex-1${showBottomNav ? " pb-[86px]" : ""}">${html}</div>${showFooter ? footerHtml() : ""}`;
  ...
  if (document.getElementById("eb-bio")) bindCharCounter("eb-bio", "eb-count", 150);
...
async function saveBio() {
  const a = state.session.account;
  const val = document.getElementById("eb-bio").value;
  const ok = await fsSetAccount(a.id, { bio: val });
  if (ok) { navigate("editAccount"); showToast("Bio updated."); } else showToast("Couldn't save — try again.", "error");
}
async function saveLink(idxStr) {
  const a = state.session.account;
  ...
  errEl.textContent = "";
  if (!url || !title) { errEl.textContent = "Both URL and title are required."; return; }
  ...
  if (ok) { navigate("editLinks"); showToast(idx >= 0 ? "Link updated." : "Link added."); } else showToast("Couldn't save — try again.", "error");
}
function deleteLink(idxStr) {
  const a = state.session.account;
  const idx = parseInt(idxStr, 10);
  askConfirm({ title: "Remove link?", message: "This link will be removed from your profile.", confirmLabel: "Remove", danger: true }, async () => {
    ...
    links.splice(idx, 1);
    const ok = await fsSetAccount(a.id, { links });
    if (ok) { navigate("editLinks"); showToast("Link removed."); } else showToast("Couldn't remove — try again.", "error");
  });
}
async function toggleAccountVisibility() {
  const a = state.session.account;
  const ok = await fsSetAccount(a.id, { profilePublic: !a.profilePublic });
  showToast(ok ? "Updated." : "Couldn't update — try again.", ok ? "success" : "error");
}
...
document.addEventListener("click", async (e) => {
  ...
  switch (action) {
    ...
    case "photo-view": state.ui.photoSheetOpen = false; state.ui.photoViewOpen = true; render(); break;
    case "close-photo-view": state.ui.photoViewOpen = false; render(); break;
    case "copy-profile-url": copyProfileUrl(id); break;
    case "toggle-account-visibility": toggleAccountVisibility(); break;
    case "save-name": saveName(); break;
    case "save-username": saveUsername(); break;
    case "save-bio": saveBio(); break;
    case "save-link": saveLink(id); break;
