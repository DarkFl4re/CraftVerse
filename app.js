...
const ICONS = {
  ...
  shieldCheck: (f, size = 18) => svgIcon(`<path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z"/><path d="m9 12 2 2 4-4"/>`, size),
...
function menuSheetHtml() {
  const items = [
    ...
    { label: "Admin Panel", id: "adminGate", icon: ICONS.shieldCheck() },
  ];
  return `<div class="fixed inset-0 bg-bgdeep z-50 overflow-y-auto">
    <div class="max-w-[480px] mx-auto">
      <div class="flex items-center gap-2.5 px-4 pt-4 pb-4">
        ${iconBtn({ action: "close-menu", icon: ICONS.arrowLeft() })}
        <div class="font-sora font-bold text-lg">Menu</div>
      </div>
      <div class="px-4 pb-8 flex flex-col gap-2.5">
        ${items.map((item) => `
          <button data-action="nav" data-id="${item.id}" class="w-full flex items-center gap-3.5 bg-panelalt border border-bd rounded-2xl px-4 py-4 text-left">
            <span class="text-tmuted flex-shrink-0">${item.icon}</span>
            <span class="flex-1 font-sora font-semibold text-[15px] text-tprimary">${item.label}</span>
            <span class="text-tfaint flex-shrink-0">${ICONS.chevronRight()}</span>
          </button>
        `).join("")}
      </div>
    </div>
...
function footerHtml() {
  ...
  return `<div class="mt-2 bg-panel border-t border-bd rounded-t-[20px] px-5 pt-7 pb-5 flex flex-col items-center gap-3">
    ${siteLogoHtml(56)}
    <div class="font-sora font-bold text-base">${esc(b.siteName)}</div>
...
function postViewScreenHtml(post) {
  ...
  links.forEach((l) => {
    ...
    html += `<a href="${escAttr(l.url || "#")}" target="_blank" rel="noopener noreferrer" class="flex items-center gap-3 bg-panel border border-bd rounded-2xl px-3.5 py-3 no-underline">
      <div class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style="background:linear-gradient(135deg,#FF5D6C,#FF9B5D);color:#fff;">${platformIcon}</div>
...
function adminGateScreenHtml() {
  ...
  <div class="px-5 pt-2 pb-8 flex flex-col gap-4">
    <button data-action="nav" data-id="ownerLogin" class="flex items-center gap-4 bg-panel border border-bd rounded-2xl p-5 text-left">
      ...
      <div><div class="font-sora font-bold text-base mb-1">Owner</div><div class="font-inter text-sm text-tmuted leading-relaxed">Full control — approve maps, manage admins</div></div>
    </button>
    <button data-action="nav" data-id="adminLogin" class="flex items-center gap-4 bg-panel border border-bd rounded-2xl p-5 text-left">
      ...
      <div><div class="font-sora font-bold text-base mb-1">Admin</div><div class="font-inter text-sm text-tmuted leading-relaxed">Submit maps, manage your own profile</div></div>
    </button>
  </div>`;
}
