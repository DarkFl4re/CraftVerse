...
function panelTopBarHtml(name, avatar, roleLabel) {
  return `<div class="flex items-center justify-between mb-5">
    ...
    <button data-action="logout" class="flex items-center gap-1.5 text-tmuted font-inter text-xs border border-bd rounded-full px-3 py-1.5 flex-shrink-0">${ICONS.logOut()} Log out</button>
  </div>`;
}
...
function ownerLoginScreenHtml() {
  ...
  <div class="px-5 pt-1.5 pb-8">
    ...
    <div class="w-full">
      ...
      ${roleBtn("owner-login", "Enter Owner panel", "owner")}
...
function adminLoginScreenHtml() {
  ...
  ${admins.length === 0
    ...
    : `<div class="px-5 pt-1.5 pb-8">
      ${roleFormIconHtml("creator", ICONS.creatorPerson())}
      ...
      ${roleBtn("admin-login", "Enter Creator panel", "creator")}
      <button data-action="forgot-password" data-id="admin" class="w-full text-center mt-3.5 bg-transparent border-none text-tmuted font-inter text-xs">Forgot password?</button>
    </div>`}`;
}
...
function renderInner() {
  ...
  app.innerHTML = page + (state.ui.exploreOpen ? exploreScreenHtml() : "") + confirmModalHtml() + toastHtml() + scrollTopHtml();
