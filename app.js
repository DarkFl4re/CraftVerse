...
function homeHeaderHtml() {
  ...
  return `<div id="site-header" class="sticky top-0 z-20 pb-1" style="background:#0A0E17;transition:background-color .2s ease, backdrop-filter .2s ease;">
    <div class="flex items-center gap-2.5 px-4 pt-4 pb-2.5">
      ...
      ${iconBtn({ action: "nav", id: "favorites", icon: ICONS.heart() })}
    </div>
  </div>`;
}
...
function gateCardHtml(role, id, title, desc, icon) {
  const c = ROLE_COLORS[role];
  return `<button data-action="nav" data-id="${id}" class="flex items-center gap-4 bg-panel border border-bd rounded-2xl p-[18px] mb-3.5 text-left w-full">
    <div class="rounded-2xl flex items-center justify-center flex-shrink-0" style="width:52px;height:52px;background:${c.soft};color:${c.c1};">${icon}</div>
    <div><div class="font-sora font-extrabold text-[15.5px] mb-1">${title}</div><div class="font-inter text-[12.5px] text-tmuted leading-relaxed">${desc}</div></div>
  </button>`;
}
function adminGateScreenHtml() {
  return `${backHeaderHtml("Login Panel")}
  <div class="px-[18px] pt-2 pb-8 flex flex-col">
    ${gateCardHtml("owner", "ownerLogin", "Owner", "Full control — approve maps, manage creators, site settings", ICONS.ownerBadge())}
    ${gateCardHtml("creator", "adminLogin", "Creator", "Submit maps, manage your own creator profile", ICONS.creatorPerson())}
  </div>`;
}
  ...
  <div class="px-5 pt-1 pb-10">
    <div class="rounded-[26px] overflow-hidden shadow-2xl">
      <div class="px-6 pt-8 pb-12 text-center" style="background:linear-gradient(135deg,#3E8EFF,#7C5CFF);">
        ...
        <div class="font-inter text-white/80 text-[12.5px]">Sign in to manage your maps</div>
      </div>
      <div class="px-6 pt-7 pb-8 bg-panel" style="margin-top:-28px;border-radius:26px 26px 0 0;">
        ${admins.length === 0
          ? `<div class="text-center text-tfaint font-inter text-sm py-6">No creator accounts yet. Ask the owner to add one.</div>`
          : `<div class="font-sora font-extrabold text-lg text-center mb-5">Login</div>
          ...
          <button data-action="forgot-password" data-id="admin" class="block text-center mb-4 mx-auto bg-transparent border-none text-tmuted font-inter text-xs">Forgot password?</button>
          ${roleBtn("admin-login", "Login", "creator")}`
        }
      </div>
    </div>
  </div>`;
...
function adminPanelHtml() {
  ...
  if (tab === "posts") {
    ...
    if (postEditorMode === "admin" && postEditorDraft) {
      ...
      body += myPosts.length === 0 ? `<div class="text-center text-tfaint font-inter text-sm py-5">You haven't submitted any maps yet.</div>` : myPosts.map(creatorMapRowHtml).join("");
      body += primaryBtn({ action: "admin-new-post", label: "Add new map", icon: `<span class="mr-1">${ICONS.plus()}</span>`, extra: "w-full mt-1" });
    }
  } else {
    body += profileEditorHtml(profileEditorDraft || account, true);
  }
  return `<div class="px-4 pt-4 pb-24">
    ${panelTopBarHtml(account.name, account.avatar, "Creator")}
    <div class="font-sora font-extrabold text-lg mb-3.5">${tab === "posts" ? "My maps" : "My profile"}</div>
    ${body}
  </div>
  ${bottomRailHtml([
...
function renderInner() {
  ...
  app.innerHTML = page + (state.ui.exploreOpen ? exploreScreenHtml() : "") + confirmModalHtml() + toastHtml();
...
async function adminLogin() {
  ...
  try {
    ...
    navigate("adminPanel");
    showToast(`Welcome back! — ${acc.name} is now signed in.`);
  } catch (e) {
